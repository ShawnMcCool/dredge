...
- [x] **Time-box: if after ~10 tool-call rounds of dependency fighting it does not produce labels, STOP**, keep novelty, record exactly what blocked it in the plan file, and move on. This task failing is an acceptable outcome.
  ...
  - **VRAM caveat:** peak ≈ 8 GiB. With a game + browsers on the 16 GiB GPU it OOMs → wrapper falls back to novelty (verified, beat grid shipped). Mitigation: `analyze_impl.py` runs the songformer subprocess *before* loading beat_this so the parent holds no VRAM. No CPU retry (would take many minutes; novelty fallback is the better UX).
  - Deftones result via wrapper: engine `songformer`, 12 sections (mostly `inst` — correct for a bass-only stem, 8-class functional labels).
- [x] Commit (either): `feat(analyze): songformer section engine` / `docs(analyze): songformer attempt notes — novelty stands`
