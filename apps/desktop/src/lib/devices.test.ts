...
import { defaultName, resolveInputDevice } from "./devices";
...
describe("resolveInputDevice", () => {
  ...
  it("returns the explicit override when one is set", () => {
    expect(resolveInputDevice("a", "b", inputs)).toBe("a");
  });

  it('"default" with a global input returns the global', () => {
    expect(resolveInputDevice("default", "a", inputs)).toBe("a");
  });

  it('"default" with no global returns the is_default device', () => {
    expect(resolveInputDevice("default", null, inputs)).toBe("b");
  });
  ...
  it('"default" with no global and none flagged returns the first input', () => {
    ...
    expect(resolveInputDevice("default", null, none)).toBe("a");
  });

  it("returns null for empty inputs with no global", () => {
    expect(resolveInputDevice("default", null, [])).toBeNull();
  });
});
