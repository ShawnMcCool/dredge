...
- **`earworm-bin`** — the one-command path. Downloads the prebuilt release
  ...
  `yay -S earworm-bin`.
- **`earworm`** — the from-source path. `makedepends` the full toolchain, clones
...
Both install the same layout: `earworm`, `earwormd`, `earworm-analyze`, and
`earworm-enable-ml` on `PATH`, the analyze Python impls under
`/usr/lib/earworm/`, the desktop entry, and the icon.

## Publishing a release to the AUR

These PKGBUILDs are templates kept in-repo; the AUR holds its own git repos
(`ssh://aur@aur.archlinux.org/earworm-bin.git`, `.../earworm.git`).
...
3. For `earworm-bin`, set `sha256sums` to the tarball's checksum from the
   release's `SHA256SUMS` (the source `earworm` package builds from the git tag,
   so it keeps `SKIP`).
...
> referenced tag/release actually exists on GitHub — `earworm-bin` needs the
> release tarball, and `earworm` clones `#tag=vX.Y.Z`. Until then these are
> validated by `bash -n` + `makepkg --printsrcinfo`. The first real release is
> the end-to-end test.

A CI step can later auto-push `earworm-bin` on each release (e.g. an
